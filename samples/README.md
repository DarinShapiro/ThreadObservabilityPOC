...
- `chat/` — reusable live-chat smoke matrices for add-on acceptance runs
	Includes `live_browser_smoke.js`, a browser-authenticated smoke helper for HA ingress sessions where terminal requests return `401 Unauthorized`. The live smoke matrix now carries AI-evaluation rubrics for the PowerShell smoke runner; the browser helper still exposes transcript artifacts but does not embed reviewer-model credentials client-side.
